...
async def chat_gpt(app, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            await message.reply_text("**Hello sir, I am Amrit. How can I help you today?**")
        else:
...
async def chat_arvis(app, message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(f"**Hello {name}, I am Amrit. How can I help you today?**")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            resp = openai.ChatCompletion.create(model=MODEL, messages=[{"role": "user", "content": query}],
...
async def chat_annie(app, message):
    try:
        ...
        name = message.from_user.first_name
        if len(message.command) < 2:
            await message.reply_text(f"**Hello {name}, I am ANIME. How can I help you today?**")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            resp = openai.ChatCompletion.create(model=MODEL, messages=[{"role": "user", "content": query}],
